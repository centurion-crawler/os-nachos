...
Thread::Finish ()
{
    (void) interrupt->SetLevel(IntOff);  //close interrupts
    ASSERT(this == currentThread);
...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) {
	scheduler->ReadyToRun(this);  //put current thread into waiting list
	scheduler->Run(nextThread);   //let next thread run
    }
    (void) interrupt->SetLevel(oldLevel);
}
